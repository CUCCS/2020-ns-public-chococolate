...
* solution:暴力枚举，得到管理员最喜欢的颜色是`green`
  ...
  ![OUTCOME](images/forget_password.png)
...
![PROCESS](images/muti_level_login2.png)
...
* solution: 先枚举找到可以执行删除个人资料操作的帐户，观察POST，发现删除简历操作标识`action=DeleteProfile`。再以tom身份登陆，执行`viewProfile`,使用`burpsuite`对ViewProfile操作的请求进行拦截并修改，转发，即可实现tom具有删除功能。
  ...
  ![PROCESS](images/delete_profile.png)
  ![OUTCOME](images/delete_profile_outcome.png)
...
* 
  ![PROCESS](images/break_layer_access.png)
  ![OUTCOME](images/break_layer_access_outcome.png)
...
![PROCESS](images/Bypass_HTML_Field_Restrictions1.png)
![PROCESS](images/Bypass_HTML_Field_Restrictions2.png)
...
![PROCESS](images/exploit_hidden_fields.png)
...
![PROCESS](images/stored_xss_attack.png)
![OUTCOME](images/stored_xss_attacks_outcome.png)
...
![PROCESS](images/CSRF_process1.png)
![PROCESS](images/CSRF_process2.png)
![OUTCOME](images/CSRF_outcome.png)
...
![PROCESS](images/command_injection.png)
![OUTCOME](images/command_injection_outcome.png)
...
* reflection：这题太坑了T——T，要求新建的txt文件位置是变化的，所以照搬网上题解的.jsp是不行的。
  ![PROCESS](images/Malicious_File_Exection1.png)
  ![PROCESS](images/Malicious_File_Exection2.png)
  ![PROCESS](images/Malicious_File_Exection_outcome.png)

**防御修复**

* 可以通过调用HTML中的`accept`属性限制上传文件的类型，自己试了试在相应的HTML代码加了该属性，发现上传文件的类型收到了限制~
  ![PROCESS](images/Malicious_File_recover.png)
  ![PROCESS](images/Malicious_File_re_outcime.png)
...
* Firefox和burpsuite设置好代理后，burpsuit无法抓取本地包？
  解决：手动搜索原因，在csdn[一篇博客](https://blog.csdn.net/weixin_45396639/article/details/104379312?utm_source=app)上看到可能是因为系统设置为不抓取本地包，按照博客在访问`about:config`后,在搜索框内搜索 `network.proxy.allow_hijacking_localhost`，将值改为true,问题解决！
